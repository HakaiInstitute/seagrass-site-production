...
colnames(zmHabitat)
```
Fix date format in both data sets
```{r}
zmDensity$date <- as.Date(zmDensity$date, format = "%m/%d/%Y")
zmHabitat$date <- as.Date(zmHabitat$date, format = "%m/%d/%Y")
```
...
zmDensity %>% 
  filter(survey == "CHOKED_PASS",  # enter the survey name, in quotes
         year(date) == 2020,  # enter year here, no quotes needed
         month(date) == 7) %>%  # enter month here, no quotes needed
  ggplot(aes(x   = factor(transect_dist_m),
